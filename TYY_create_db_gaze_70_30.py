...
def get_args():
	...
	parser.add_argument("--db", type=str, default='./GazeDatasets',
	                    help="path to database")
	parser.add_argument("--output", type=str, default='./GazeDatasets.npz',
...
def main():
	...
	for num in range(0,15):
		malenum = mypath+'/MaleBlack'+str(num+1)
		for num in range(0,8):
			mypath_obj = malenum+'/cam'+str(num+1)
			print(mypath_obj)
			onlyfiles_txt_temp = [f for f in listdir(mypath_obj) if isfile(join(mypath_obj, f)) and join(mypath_obj, f).endswith('final.txt')]
			onlyfiles_png_temp = [f for f in listdir(mypath_obj) if isfile(join(mypath_obj, f)) and join(mypath_obj, f).endswith('final.png')]
	...
	for i in range(len(onlyfiles_png)):
		...
		# print('Camera in a MaleBlack %d' %i)
		...
		if cam==0:
			cam=8
			male=male-1
		mypath_obj = mypath+'/MaleBlack'+str(int(male))+ '/cam'+str(cam)
